    ...
    return (
        <>
            {props.accountdata.map((value, index) => (
                <div key={index}>
                    <a href={value.link} className="transition ease-out text-neutral-50 rounded-xl duration-300 w-10 h-10 bg-white/0 hover:bg-white/10 shadow-[inset_1.5px_1.5px_4px_0px_#000] flex justify-center items-center overflow-hidden relative hover:text-black" title={value.name}>
